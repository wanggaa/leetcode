...
# 
# 如果单词列表（words）中的一个单词包含牌照（licensePlate）中所有的字母，那么我们称之为 补全词 。在所有完整词中，最短的单词我们称之为
# 最短补全词 。
# 
# 单词在匹配牌照中的字母时要：
# 
...
# 
# 
# 例如：licensePlate = "aBc 12c"，那么它由字母 'a'、'b' （忽略大写）和两个 'c' 。可能的 补全词 是
...
# 输出："pest"
# 说明：存在 3 个包含字母 "s" 且有着最短长度的补全词，"pest"、"stew"、和 "show" 三者长度相同，但我们返回最先出现的补全词
# "pest" 。
# 
# 
# 示例 3：
...
class Solution:
    def shortestCompletingWord(self, licensePlate: str, words: List[str]) -> str:
        ...
        for c in license:
            if not c.isalpha():
                continue
            l_dict[c] = l_dict.get(c,0) + 1

        words = sorted(words,key = lambda x:len(x))
        for word in words:
            symbol = True
            for k in l_dict.keys():
                if word.count(k) < l_dict[k]:
                    symbol = False
                    break
            if symbol:
                return word
