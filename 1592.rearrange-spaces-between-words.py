...
class Solution:
    def reorderSpaces(self, text: str) -> str:
        ...
        alpha_num = sum([len(word) for word in words])
        space_num = len(text) - alpha_num
        if len(words) <= 1:
            return ''.join(words) + space_num * ' '
        avg_num = space_num // (len(words)-1)
        ans = (' '*avg_num).join(words)
        ans += (space_num - avg_num * (len(words)-1)) * ' '
        return ans
